...
public class Program
{
    static int Main()
    {
        ...
        Console.WriteLine("It is first lab");
        ...
        obj.GetCompanyName();
        Console.WriteLine($"Число клиентов компании в шестнадцатиричной системе счисления: { (obj.CountOfUsers is null ? "<unset>": Convert.ToString((int) obj.CountOfUsers, 8))}");
        ...
        while (true)
        {
            ...
            string[] raw_data;
            if ((raw_data = inputData.Split(" ")).Length > 1 && obj.SetSomeValue(raw_data[0],
                    string.Join(" ", raw_data.Where((x, index) => index != 0))))
            {
                Console.WriteLine(obj.ToString());
            }
            else
